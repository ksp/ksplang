...
pub enum PrecompiledOp<TReg: Display> {
    ...
    CurvedDiv(TReg, TReg, TReg), // a <- b % c if non-zero, otherwise b / c
    ...
    DigitSumDigitSumLensum(TReg, TReg), // a <- lensum(digit_sum_len(digit_sum(b)), digit_sum(b))
    Gcd(TReg, TReg, TReg), // a <- gcd(b, c)

    StackSwap(TReg, TReg, TReg, u8), // a <- stack[b]; stack[b] <- c; if b + d < stack_size, otherwise deopt
...
impl<TReg: Display> PrecompiledOp<TReg> {
        ...
        where F: Fn(&TReg, bool) -> TReg2
    {
        match self {
            ...
            PrecompiledOp::CurvedDiv(a, b, c) => PrecompiledOp::CurvedDiv(f(a, true), f(b, false), f(c, false)),
...
pub struct PrecompiledBlock {
    pub program: Box<[PrecompiledOp<u32>]>,
